...
export function validateArticle(article, match, topic, options = {}) {
  const minWords = options.minWords ?? 1150;
  const maxWords = options.maxWords ?? 1800;
  ...
  if (article.sections.length < 6) {
    throw new Error("Article must include at least 6 sections");
  }

  const allText = JSON.stringify(article).toLowerCase();

  if (match.home_team !== "World Cup" && !allText.includes(match.home_team.toLowerCase())) {
    throw new Error("Article does not mention home team");
  }

  if (match.away_team !== "2026" && !allText.includes(match.away_team.toLowerCase())) {
    throw new Error("Article does not mention away team");
  }

  const totalWords = getArticleWordCount(article);
  ...
  if (totalWords > maxWords) {
    throw new Error(`Article too long: ${totalWords} words`);
  }

  return {
    title: String(article.title).slice(0, 92),
    description: String(article.description).slice(0, 170),
    intro: String(article.intro || "").slice(0, 1600),
    ...
    faqs: Array.isArray(article.faqs)
      ? article.faqs.slice(0, 4).map((faq) => ({
          question: String(faq.question || "").slice(0, 150),
          answer: String(faq.answer || "").slice(0, 1000)
        }))
      : []
  };
}